...
export default function WordCard({ content }: WordCardProps) {
	return (
		<div className='relative grid place-items-center h-24 w-[60%] bg-[var(--bg-translated-card)]'>
			...
			<p className='text-zinc-300 capitalize'>{content}</p>
			<SoundIcon className='absolute bottom-1 right-1 w-5 cursor-pointer text-zinc-400 hover:text-[var(--contrast-color)]' />
			{/* <p className='text-sm absolute bottom-1 right-7 w-5 cursor-pointer text-zinc-400 hover:text-[var(--contrast-color)]'>Listen</p> */}
		</div>
	);
}
